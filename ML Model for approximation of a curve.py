...
parameters = np.array([14.7, -8.3, 0.5, 100.0]) # parameter [a, b, c, d]
...
def Grad_descent(data):
    update_parameters = np.array([0.0, 0.0, 0.0, 0.0])
...
for i in range(0,1000):
    parameters -= Grad_descent(training_data)
    print(calculate_MSL(training_data, parameters))


pred_x = np.array(range(1, 361))
pred_y = parameters[0] * pred_x**3 + parameters[1] * pred_x**2 + parameters[2]*pred_x + parameters[3]
